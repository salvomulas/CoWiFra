...
class dashboard_loadtest extends CI_Controller {
	...
	public function index() {
        //if ($this->session->userdata('is_logged_in')) {
        	...
        	if ($query_projects) {
        		...
        		echo "Projekte konnten nicht aus der Datenbank gelesen werden.";
        	}
//        } else {
    ...
    public function new_project() {
        if ($this->session->userdata('is_logged_in')) {
            ...
            if ($query_users) {
                ...
                $body_data['error_message'] = "Benutzer konnten nicht aus der Datenbank gelesen werden.";
                $this->load->view('header');
                $this->load->view('meta', $meta_data);
                $this->load->view('new_project_success', $body_data);
            }
                    ...
                    if ($insert_id) {
                        ...
                        foreach ($user_array as $user_string) {
                            ...
                            if ($user_id and ($is_admin == 'false' or $is_admin == 'true')) {
                                ...
                                $body_data['success_message'] = 'Das Projekt "' . htmlspecialchars($this->input->post('project_name')) . '" wurde erfolgreich erstellt.';
                                $body_data['error_message'] = 'Fehler beim Berechtigen der Benutzer f&uuml;r das Projekt.';
                            }
                        }
                        ...
                        $body_data['error_message'] = 'Fehler beim Erstellen des Projekts.';
                    }
